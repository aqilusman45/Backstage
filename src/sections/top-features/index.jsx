...
export default function TopFeatures({ otherClasses }) {
  ...
  const { featureSections, plans } = homePageMocks;
              ...
              {featureSections.map((section, sectionIndex) => (
                <tr
                  ...
                  className={classnames(
                    "border-t border-black/10  bg-[#FCFCFD] py-5",
                    section?.header ? "bg-[#FCFCFD]" : "bg-white",
                  )}
                >
                  ...
                  {plans.map((plan, planIndex) => (
                    <td key={planIndex} className="p-4 text-center">
                      {section.isBoolean ? (
                        plan[section.key || "features"][sectionIndex] ? (
                          <Image
                        ...
                        <span className="px-4 block font-normal font-instrument-sans text-base text-black/60 block">
                          {plan[section.data]}
                        </span>
                      )}
                    </td>
